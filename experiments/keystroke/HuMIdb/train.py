    ...
    for user in data:
        for session in user:
            for sequence in session:
                if sequence[0].dtype != "float64":
                    sequence[0] = sequence[0].astype("float64")
                if sequence[1].dtype != "float64":
                    sequence[1] = sequence[1].astype("float64")
        ...
        for session in user:
            for index in range(len(session)):
                ...
                for feature in range(36):
                    ...
                    elif feature in [3, 4, 5, 24, 25, 26, 27, 28, 29]:
                        imu[:, feature] = imu[:, feature] / 1000
                    elif feature in [15, 16, 17]:
...
if __name__ == "__main__":
    ...
    preprocessed_data = config["preprocessed_data"]["humi"]
    ...
    enrollment_sessions = hyperparams["number_of_enrollment_sessions"]["humi"]
    verify_sessions = hyperparams["number_of_verify_sessions"]["humi"]
